...
typedef struct _AdminInfo {
	...
	char adminVenueID[65];				//管理员管理的场馆ID，6-10个字符
} AdminInfo;
...
typedef struct _SiteInfo {
	...
	char siteName[65];					//场地名称，15个字符以内
	char siteLocation[256];				//场地位置，20个字符以内
	char siteVenueID[65];				//场地所属的场馆ID，6-10个字符
	char siteSportsType[65];			//场地的运动类型，64个字符以内
	char siteIntro[256];				//场地简介，100个字符以内
	double siteRent;					//场地每小时租金，双精度浮点
	int siteAllowedAgeMin;				//场地最小准入年龄，整型
	int siteAllowedAgeMax;				//场地最大准入年龄，整型
